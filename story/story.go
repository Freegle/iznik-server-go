...
func UpdateStory(c *fiber.Ctx) error {
	...
	type UpdateRequest struct {
		...
		Newsletterreviewed *int    `json:"newsletterreviewed"`
	...
	if req.Reviewed != nil {
		db.Exec("UPDATE users_stories SET reviewed = ?, reviewedby = ? WHERE id = ?", *req.Reviewed, myid, req.ID)
	}
	if req.Newsletterreviewed != nil {
		db.Exec("UPDATE users_stories SET newsletterreviewed = ?, newsletterreviewedby = ? WHERE id = ?", *req.Newsletterreviewed, myid, req.ID)
	}
