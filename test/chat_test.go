...
func TestChatSearchDoesNotReturnOldChats(t *testing.T) {
	// V1 parity: chat search only returns chats within the active time window
	// (31 days). Old chats matching the search term should NOT appear.
	prefix := uniquePrefix("SearchOld")
	db := database.DBConn
	...
	// Create a User2User chat with a message containing a searchable term.
	chatID := CreateTestChatRoom(t, userA, &userB, nil, "User2User")
	db.Exec("INSERT INTO chat_messages (chatid, userid, message, date, processingsuccessful, reviewrequired, reviewrejected) VALUES (?, ?, 'I have a wonderful bookcase for you', NOW(), 1, 0, 0)",
		chatID, userA)

	// Set latestmessage to 60 days ago — outside the 31-day window.
	db.Exec("UPDATE chat_rooms SET latestmessage = DATE_SUB(NOW(), INTERVAL 60 DAY) WHERE id = ?", chatID)

	// User A searches for "bookcase" — should NOT find this old chat.
	_, tokenA := CreateTestSession(t, userA)
	req := httptest.NewRequest("GET", fmt.Sprintf("/api/chat?includeClosed=true&search=bookcase&jwt=%s", tokenA), nil)
	resp, _ := getApp().Test(req)
	assert.Equal(t, 200, resp.StatusCode)
	...
	for _, c := range chats {
		assert.NotEqual(t, float64(chatID), c["id"], "Old chat should not appear in search results")
	}

	// Now set latestmessage to today — should appear.
	db.Exec("UPDATE chat_rooms SET latestmessage = NOW() WHERE id = ?", chatID)

	req2 := httptest.NewRequest("GET", fmt.Sprintf("/api/chat?includeClosed=true&search=bookcase&jwt=%s", tokenA), nil)
	resp2, _ := getApp().Test(req2)
	assert.Equal(t, 200, resp2.StatusCode)

	var chats2 []map[string]interface{}
	json2.Unmarshal(rsp(resp2), &chats2)
	...
	assert.True(t, found, "Recent chat with matching message should appear in search results")
}
