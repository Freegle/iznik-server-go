...
func GetGroupMessages(c *fiber.Ctx) error {
	...
	db.Raw("SELECT messages_groups.msgid FROM messages_groups "+
		...
		"WHERE groupid = ? AND arrival >= ? AND (collection = ? OR messages.fromuser = ?) AND deleted = 0 AND (messages_outcomes.id IS NULL OR messages_outcomes.outcome IN (?, ?)) "+
		"ORDER BY arrival DESC", id, then.Format(time.RFC3339), utils.COLLECTION_APPROVED, myid, utils.OUTCOME_TAKEN, utils.OUTCOME_RECEIVED).Pluck("msgid", &ret)
