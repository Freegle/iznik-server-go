...
func GetUserUinfo(id uint64) UserInfo {
	...
	go func() {
		...
		db.Raw("SELECT COUNT(DISTINCT refmsgid) AS replies FROM chat_messages WHERE userid = ? AND date > ? AND refmsgid IS NOT NULL AND type = ?", id, start, utils.CHAT_MESSAGE_INTERESTED).Scan(&info)
	}()

	wg.Add(1)
	go func() {
		defer wg.Done()
		db.Raw("SELECT COUNT(DISTINCT(messages_reneged.msgid)) AS reneged FROM messages_reneged WHERE userid = ? AND timestamp > ?", id, start).Scan(&info)
	}()

	wg.Add(1)
	go func() {
		defer wg.Done()
		db.Raw("SELECT COUNT(DISTINCT messages_by.msgid) AS collected FROM messages_by "+
